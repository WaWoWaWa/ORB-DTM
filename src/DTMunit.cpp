...
vector<DMatch> ComputeDTMunit(int threshold, const vector<DMatch> &initGood_matches, const vector<cv::KeyPoint> &mvKeys1, const vector<cv::KeyPoint> &mvKeys2, cv::Mat &feature1, cv::Mat &feature2 )
{
    ...
    Mat feature3_ = feature1.clone();
    Mat feature4_ = feature2.clone();
    ///delaunay one
//    cout << "DT one:" << endl;
...
//        {
//            cout << (edgeMatrix.cwiseAbs().colwise().sum())(0,i-1) << "\t, "
//                 << mvKeys1[newGood_matches[i-1].queryIdx].pt <<"\t, " << mvKeys2[newGood_matches[i-1].trainIdx].pt << endl;
